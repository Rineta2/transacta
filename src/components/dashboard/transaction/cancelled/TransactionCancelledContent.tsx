...
export default function TransactionCancelledContent() {
    const [transactions, setTransactions] = useState<Transaction[]>([])
    const [loading, setLoading] = useState(true)
    const [selectedTransaction, setSelectedTransaction] = useState<Transaction | null>(null)
    const [isModalOpen, setIsModalOpen] = useState(false)

    // Simplified state variables - removed statusFilter
    const [currentPage, setCurrentPage] = useState(0)
    const [searchTerm, setSearchTerm] = useState('')
    const itemsPerPage = 12

    // Simplified filtering - removed status filter
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            try {
                ...
                // Filter for cancelled transactions and sort by createdAt
                const cancelledTransactions = transactionData
                    .filter(transaction => transaction.status.toLowerCase() === 'cancelled')
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 animate-fade-in">
                    <div className='flex flex-col gap-1.5'>
                        ...
                        <p className="text-sm md:text-base text-gray-600">
                            View and manage your cancelled transactions
                        </p>
                    </div>
                </div>

                {/* Simplified Filter Controls - only search remains */}
                <div className="mb-6">
                    <input
                        type="text"
